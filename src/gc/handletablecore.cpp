...
void SegmentPreCompactAsyncPinHandles(TableSegment *pSegment)
{
    ...
    pSegment->rgFreeCount[(int)HandleType::AsyncPinned] = freeCount;
}
...
BOOL SegmentCopyAsyncPinHandle(TableSegment *pSegment, _UNCHECKED_OBJECTREF *h)
{
    ...
    if (pSegment->rgFreeCount[(int)HandleType::AsyncPinned] == 0)
    {
        ...
        pSegment->rgBlockType[uBlock] = (uint8_t)HandleType::AsyncPinned;
        pSegment->rgAllocation[uBlock] = pSegment->rgHint[(int)HandleType::AsyncPinned];
        pSegment->rgHint[(int)HandleType::AsyncPinned] = uBlock;
        pSegment->rgFreeCount[(int)HandleType::AsyncPinned] += HANDLE_HANDLES_PER_BLOCK;
    }
    uint8_t uBlock = pSegment->rgHint[(int)HandleType::AsyncPinned];
    ...
    pSegment->rgFreeCount[(int)HandleType::AsyncPinned] --;
    return TRUE;
}
...
void SegmentCompactAsyncPinHandles(TableSegment *pSegment, TableSegment **ppWorkerSegment)
{
    ...
    uint32_t uBlock = pSegment->rgHint[(int)HandleType::AsyncPinned];
    if (uBlock == BLOCK_INVALID)
    {
        return;
    }
    for (uBlock = 0; uBlock < pSegment->bEmptyLine; uBlock ++)
    {
        ...
        do
        {
            ...
            if (fNeedNewSegment)
            {
                _ASSERTE ((*ppWorkerSegment)->rgFreeCount[(int)HandleType::AsyncPinned] == 0 &&
