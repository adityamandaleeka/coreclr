...
PTRARRAYREF DispatchMemberInfo::GetParameters()
{
    ...
    switch (GetMemberType())
    {
        case Method:
        {
            OBJECTREF memberInfo = ObjectToOBJECTREF(pHandleTable->ObjectFromHandle(m_hndMemberInfo));
            pGetParamsMD = DispatchInfo::GetMethodInfoMD(METHOD__METHOD__GET_PARAMETERS, memberInfo->GetTypeHandle());
        ...
        {
            OBJECTREF memberInfo = ObjectToOBJECTREF(pHandleTable->ObjectFromHandle(m_hndMemberInfo));
            pGetParamsMD = DispatchInfo::GetPropertyInfoMD(METHOD__PROPERTY__GET_INDEX_PARAMETERS, memberInfo->GetTypeHandle());
    ...
    if (pGetParamsMD != NULL)
    {
        ...
        { 
            ObjToArgSlot(ObjectToOBJECTREF(pHandleTable->ObjectFromHandle(m_hndMemberInfo)))
        };
...
void DispatchMemberInfo::DetermineMemberType()
{
    ...
    OBJECTREF MemberInfoObj = ObjectToOBJECTREF(GCHeapUtilities::GetGCHandleTable()->ObjectFromHandle(m_hndMemberInfo));
...
void DispatchMemberInfo::DetermineParamCount()
{
    ...
    OBJECTREF MemberInfoObj = ObjectToOBJECTREF(pHandleTable->ObjectFromHandle(m_hndMemberInfo));
    GCPROTECT_BEGIN(MemberInfoObj);
    {
        // Retrieve the method to use to retrieve the array of parameters.
        switch (GetMemberType())
        {
            case Method:
            {
                pGetParamsMD = DispatchInfo::GetMethodInfoMD(METHOD__METHOD__GET_PARAMETERS, ObjectToOBJECTREF(pHandleTable->ObjectFromHandle(m_hndMemberInfo))->GetTypeHandle());
                _ASSERTE(pGetParamsMD && "Unable to find method MemberBase::GetParameters");
                break;
            }
            ...
            case Property:
            {
                pGetParamsMD = DispatchInfo::GetPropertyInfoMD(METHOD__PROPERTY__GET_INDEX_PARAMETERS, ObjectToOBJECTREF(pHandleTable->ObjectFromHandle(m_hndMemberInfo))->GetTypeHandle());
                _ASSERTE(pGetParamsMD && "Unable to find method PropertyInfo::GetIndexParameters");
                break;
            }
        }
...
DispatchMemberInfo* DispatchInfo::FindMember(SString& strName, BOOL bCaseSensitive)
{
    ...
    {
        if (ObjectToOBJECTREF(GCHeapUtilities::GetGCHandleTable()->ObjectFromHandle(pCurrMemberInfo->m_hndMemberInfo)) != NULL)
        {
                                      ...
                                      VARIANT**             aByrefArgOleVariant)
{
    ...
    if (!m_bInvokeUsingInvokeMember)
    {
        ...
        pObjs->MemberInfo = ObjectToOBJECTREF(GCHeapUtilities::GetGCHandleTable()->ObjectFromHandle(pDispMemberInfo->m_hndMemberInfo));
            ...
            for (UINT i = 0; i < NumComponents; i++)
            {
                ...
                GCPROTECT_BEGIN(CurrMemberInfoObj)
                {
                    ...
                    while (pCurrMemberInfo)
                    {
                        ...
                        if (CurrMemberInfoObj == ObjectToOBJECTREF(GCHeapUtilities::GetGCHandleTable()->ObjectFromHandle(pCurrMemberInfo->m_hndMemberInfo)))
                        {
...
OBJECTREF DispatchInfo::GetOleAutBinder()
{
    ...
    if (m_hndOleAutBinder)
        return ObjectToOBJECTREF(GCHeapUtilities::GetGCHandleTable()->ObjectFromHandle(m_hndOleAutBinder));
...
void DispatchExInfo::DeleteMember(DISPID DispID)
{
    ...
    // Take a lock before we check that the member has not already been deleted.
    {
        ...
        if (pDispMemberInfo && (ObjectToOBJECTREF(pHandleTable->ObjectFromHandle(pDispMemberInfo->m_hndMemberInfo)) != NULL))
        {
            OBJECTREF TargetObj = GetReflectionObject();
            GCPROTECT_BEGIN(TargetObj);
            ...
            OBJECTREF MemberInfoObj = ObjectToOBJECTREF(pHandleTable->ObjectFromHandle(pDispMemberInfo->m_hndMemberInfo));
